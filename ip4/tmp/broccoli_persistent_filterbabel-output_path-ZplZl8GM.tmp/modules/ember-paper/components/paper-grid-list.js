...
export default Component.extend({
  ...
  _watchResponsiveAttributes: function _watchResponsiveAttributes(attrNames, watchFn) {
    ...
    attrNames.forEach(function (attrName) {
      if (get(_this2, attrName)) {
        ...
        var customObserver = run.bind(_this2, checkObserverValues, _this2, attrName);
      ...
      for (var mediaName in _this2.get('constants.MEDIA')) {
        ...
        if (get(_this2, normalizedName)) {
          var customObserverNormalized = run.bind(_this2, checkObserverValues, _this2, normalizedName, mediaName);
  ...
  _layoutDelegate: function _layoutDelegate(tilesInvalidated) {
    ...
    gridLayout(props.colCount, props.tileSpans, tiles).map(function (tilePositions, rowCount) {
      return {
        grid: {
          element: _this4.$(),
